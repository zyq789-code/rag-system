<template>
  <div class="flex flex-col h-full">
    <!-- 顶部栏 -->
    <div class="flex items-center justify-between px-6 py-3 border-b border-gray-100 bg-white/80 backdrop-blur-sm">
      <h2 class="font-semibold text-gray-800" style="font-family: var(--font-heading)">智能问答</h2>
      <div class="flex items-center gap-3">
        ...
        <select
          ...
          class="text-sm border border-gray-200 rounded-lg px-3 py-1.5 text-gray-600 bg-gray-50 focus:bg-white focus:border-indigo-300 transition-colors"
        >
        ...
        <button @click="store.clearChat()" class="text-sm text-gray-400 hover:text-gray-600 transition-colors px-2 py-1 rounded hover:bg-gray-100">
          新对话
        </button>
      </div>
    </div>
    ...
    <div class="border-t border-gray-100 bg-white px-6 py-4">
      <div class="flex gap-3 max-w-4xl mx-auto">
        <textarea
          ...
          placeholder="输入问题..."
          class="flex-1 resize-none border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-indigo-300 focus:ring-1 focus:ring-indigo-300 transition-colors bg-gray-50 hover:bg-white focus:bg-white"
          rows="1"
        />
        <button
          @click="handleSend"
          :disabled="!input.trim() || store.isStreaming"
          class="px-5 py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white rounded-xl hover:from-indigo-600 hover:to-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed text-sm font-medium transition-all shadow-sm hover:shadow"
        >
          发送
        </button>
      </div>
